...
internal partial class CategoriesViewModel : ObservableCollection<Category>
{
    ...
    // bcause they are set programmatically to display to user.
    ...
    public CategoriesViewModel() 
    {
        try
        {
            ...
            IEnumerable<Category> categories = response.Content.ReadFromJsonAsync<IEnumerable<Category>>().Result;

            foreach (Category category in categories) 
            {
                int offset = 78; // to remove the OLE header

                category.Picture = category.Picture.AsSpan(offset, category.Picture.Length - offset).ToArray();
